...
export function LittleKrishna({
  ...
  // Derive default state if stateProp is not directly supplied
  ...
  // Randomized non-robotic blink timer
  useEffect(() => {
    let blinkTimer: NodeJS.Timeout;
    let holdTimer: NodeJS.Timeout;

    const triggerBlinkCycle = () => {
      const nextDelay = Math.random() * 3500 + 2500; // 2.5s to 6.0s random interval
      blinkTimer = setTimeout(() => {
        setIsBlinking(true);
        holdTimer = setTimeout(() => {
          setIsBlinking(false);
          triggerBlinkCycle();
        }, 140); // 140ms blink hold
      }, nextDelay);
    };
  ...
  const triggerTemporaryState = (tempState: KrishnaState, durationMs: number) => {
    ...
    const baseState = stateProp || (pose === 'chakra' ? 'protector' : 'idle');
  ...
  // State-specific speech bubble messages
  ...
  return (
    ...
    >
      ...
      {/* External HTML Thought Bubble for Thinking State */}
      ...
      <div className={`${styles.krishna} ${getStateClass()}`}>
        ...
        >
          ...
          <g id="dhoti" filter="url(#kSoftShadow)">
            ...
            <g transform="translate(216, 274)" id="waistSashKnot">
          ...
          <g id="headGroup" transform="translate(0, -18)">
            ...
            <g id="faceDetails">
              ...
              <g transform="translate(130, 137)">
                ...
                {/* Upper Localized Eyelid Blink Layer */}
                <path
                  d="M 0 14 C 7 3, 29 3, 36 14 C 29 9, 7 9, 0 14 Z"
                  fill="#4F8EF0"
...
export default function KrishnaSprite(props: BuddySpriteProps & { state?: KrishnaState; size?: 'sm' | 'md' | 'lg' }) {
  return (
    <LittleKrishna
      state={props.state}
      mood={props.mood}
